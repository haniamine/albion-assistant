...
def select_journal(
    ...
    if journal_match is None:
        print(f"journal image not found for {laborer_name}.")
        return False

    journal_position, score = journal_match
    print(f"journal position for {laborer_name}: {journal_position}.")
...
def main() -> None:
    ...
    print("Listening for '&' / '1'. Press Esc to stop.")
    print(f"Loaded {len(mapping)} laborer mapping(s).")

    last_action = 0.0
    while True:
        if is_pressed(win32con.VK_ESCAPE):
            print("Stopped.")
            return

        if is_pressed(VK_1) or is_pressed(VK_NUMPAD1):
            now = time.monotonic()
                ...
                try:
                    ...
                    if ready_monitor_index is not None:
                        ...
                        if laborer_match is not None:
                            ...
                            if select_journal(laborer_name, monitor_index, journal_templates):
                                click_accept(accept_template)
                finally:
                    time.sleep(RESTORE_MOUSE_DELAY_SECONDS)
                    win32api.SetCursorPos(original_mouse_position)
                    print(f"restored mouse position to {original_mouse_position}.")
                    last_action = now
